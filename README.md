...
Rechercher les fichiers suivantes :
...
### 1.4 Analyse des techno utilisées
...
Il y a donc toutes les techno trouvées avec des informations précises sur leur version, ce à quoi elles servent, etc. On pourra ensuite se baser sur les technos détectées pour la phase [Phase 2](#phase-2-recherche-et-exploitation-de-vulnerabilites).
Si certaines techno ne sont pas à jour : 
```
...
On a déjà une idée des vulnérabilités de l'application grâce au scan `wapiti` mais il peut être intéressant de tester les autres aussi dans le cas où il n'aurait pas trouver la faille.
...
On peut de toutes façons commencer par rechercher des fichiers sensibles qui peuvent être accessibles via des URL prévisibles. [Dirb et Wfuzz](#17-liste-des-dossiers-et-fichiers) nous ont donné des informations importantes sur où les chercher.

Si on voit :

- un champs d'entrée utilisateur (recherche, commentaire, etc) :
...
On peut facilement tester si le site est vulnérable aux injection XSS en injectant une *string* arbitraire comme `abc123` dans un point d'entrée et en cherchant dans le code de la page où notre *input* a été injectée. La phase d'[analyse des sources](#13-analyse-du-code-source) nous permet de plus facilement retracer le chemin des données.
...
- récupérer, dans le code, toutes les données utiles à son bon fonctionnement (nom des champs, valeurs des token `csrf` (souvent dans des [champs cachés](#13-analyse-du-code-source)) ou des cookies).
...
**A noter** : dans les sections suivantes nous allons tenter d'injecter des payloads dans notre application mais certaines sécurités devraient être en place pour bloquer les mots-clés SQL (`SELECT`, `UNION`, `WHERE`, ...) mais gardez en tête que SQL n'est pas regardant sur la casse des requêtes et que ces restrictions peuvent être faite avec des *whitelists*, on peut donc toujours essayer d'autres casses (`Select`, `SeLeCt`, ...)
...
On devrait obtenir le nom des tables de la base. A partir de là on peut facilement monter un payload en `'UNION SELECT ... FROM ... WHERE ...'`. Pour tirer des informations de la base entière.
